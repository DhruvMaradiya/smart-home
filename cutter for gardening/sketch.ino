...
const char* machineTopic = "cutter/machine";
  ...
  if (machineState) {
    ...
    if (distanceFront <= DIST_THRESHOLD) {
      if (distanceLeft > distanceRight) {
        Serial.println("Grass on the left, moving left");
        moveLeft();
      } else {
        Serial.println("Grass on the right, moving right");
        moveRight();
      }
    } else if (distanceBack <= DIST_THRESHOLD) {
      Serial.println("Grass behind, turning around");
      turnAround();
    } else {
      Serial.println("No Grass detected. Moving forward.");
      moveForward();
    }
